...
updatePersonnalClasses(session)#PK DA ERRO DE UNICIDADE NA METATABELA?????
session.close()
...
def showgoal():
    result=show_goal(Person)
    return "The goal of the data of class person is {0} ".format(result)

#   TEM DE TER ALGUMA PESSOA NA TABELA
@route('/showcategorie')   # lista goal
def showcategorie():
    result=show_categorie(Person)
...
def showdatasource():
    result=show_data_source(Person)
    return "The source of the data of class person is {0} ".format(result)
...
def create():
    ...
    session.add(restaurant)
    session.commit()
    checkin = Checkin(30,31 , 30 , "blabla", 3)  # id_c, id, id_r, description, rating
    session.add(checkin)
    session.commit()
    ...
    session.add(employee)
    session.commit()
    schedule = Schedule(40,40,31,datetime.now().replace(microsecond=0)+timedelta(hours=1),datetime.now().replace(microsecond=0)+timedelta(hours=8))  # id_s, id_e, id_r, checkin,checkout
