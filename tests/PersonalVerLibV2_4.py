...
class Roots (Base):
    __tablename__ = 'roots'
    id_secu= Column('id_sec', Integer, primary_key=True, unique=True)
    l_pessoal= Column('pessoal', String, unique=True )
...
def updatePersonnalClasses(session):
    ...
    for y in rootClasses.keys():
        try:                                                        #tentar retirar o try: e a parte do except
            data = Roots(y)
            session.add(data)
            session.commit()
        except:
            print "Warnig updateRootClasses: Already exists in Roots"
############################################################################################################################################################################################
...
def Personal_tag_router(classe):
    ...
    Session = sessionmaker(bind=engine)
    session= Session()
    data= session.query(Roots).all()
    for x in data:
    #for t in engine.table_names():
    #None part is useless TIRAR
        if(find_shortest_path(grafo, classe.__tablename__, x.l_pessoal) is None):
            pass
        elif(((len(find_shortest_path(grafo, classe.__tablename__,x.l_pessoal))==1))and(n==100000)):      #tinha um if((find_shortest_path(grafo, classe.__tablename__, x.l_pessoal) is None) or(len(find_shortest_path(grafo, classe.__tablename__,x.l_pessoal))==1))and(n==0)):
